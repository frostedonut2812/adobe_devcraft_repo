...
class RTB2(Dataset):
    def __init__(self, impressions_file_path, click_file_path, conversion_file_path, split="train"):
        ...
        self.creative_id_mapping = { 
            'e1af08818a6cd6bbba118bb54a651961': 0, '44966cc8da1ed40c95d59e863c8c75f0': 1, 
            '832b91d59d0cb5731431653204a76c0e': 2, '59f065a795a663140e36eec106464524': 3, 
            '48f2e9ba15708c0146bda5e1dd653caa': 4
        }
        ...
        self.imp.drop(columns=['BidID', 'Logtype', 'VisitorID', 'User-Agent', 'IP', 
                               'Adexchange', 'Domain', 'URL', 'AnonymousURLID', 'AdslotID', 
                               'Adslotfloorprice', 'Biddingprice', 'KeypageURL', 'Timestamp'], inplace=True)

        # Train-validation split
        stratify_label = self.clk_label
        self.imp_train, self.imp_val, self.clk_train, self.clk_val, self.conv_train, self.conv_val = train_test_split(
            self.imp, self.clk_label, self.conv_label, train_size=0.05, stratify=stratify_label
        )
        ...
        print(f"Train dataset size: {len(self.imp_train)}")
        print(f"Validation dataset size: {len(self.imp_val)}")
...
if __name__=='__main__':
    ...
    dataset = RTB2(imp_file_path,clk_file_path,conv_file_path, split='train')
